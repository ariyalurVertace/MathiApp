...
class EbeatLocationTypeModal extends React.Component {
    state = {
    ...
    componentWillReceiveProps() {
        let {locationTypes, locationType} = this.props;

        if (locationType.length) {
            let index = locationTypes.findIndex(a => a.label === locationType[0].label);
            this.setState({index});
        }
    }
    ...
    handleClose = (isSave = false) => {
        ...
        details.selectedLocationType = this.state.selectedLocationType;

        closeModal({details, isSave});
    };

    renderList = () => {
        let {locationTypes} = this.props;
        return (
            <RadioForm
                radio_props={locationTypes}
                ...
                animation={true}
                onPress={value => {
                    this.setState({selectedLocationType: value});
...
export default EbeatLocationTypeModal;
